...
コンピュータ・電磁的記録対象犯罪では、サーバシステム等に保管されているデータを不正に書き換える行為や、クレジットカードの偽造や偽造を目的として不正に情報を取得する行為が含まれる。たとえば不正に書き換える行為では、サーバシステム上の脆弱性を悪用しアクセス制限を迂回してデータの書き換えを行うことなどが具体例としてあげられる。
...
企業利用のPCであれば、さまざまな企業内システムへのアクセスIDとパスワードや、企業内の機密情報などがある。
...
[3] マルウェア対策研究人材育成ワークショップ（MWS）, https://www.iwsec.org/mws/
