...
ECBモードは最も単純な暗号利用モードであり、それぞれのブロックが独立して暗号化される。
...
その他のモードについては、秘匿性を高めるためにブロック間に連携を持たせる工夫などがされている。たとえばCBCモードでは1つ前の暗号文ブロックと平文ブロックの排他的論理和（XOR）を計算した後に暗号化されるために、同じ平文ブロックであっても1つ前のブロックの情報により同じ暗号文ブロックになる可能性が低くなる。
